...
Validate that a real workspace can use the Artifact Trust Layer without changing Kit's six-architecture model.
...
- No platform-native action is delegated to Kit.
...
If this test passes, the next question is whether Kit needs programmatic workspace generation for module attachment. The current setup script only tracks setup state. Do not add copying behavior to `scripts/setup_state.py` unless the setup engine becomes responsible for workspace materialization.
